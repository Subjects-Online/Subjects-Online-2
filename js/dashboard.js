...
function initActivityChart() {
    ...
    // 1. Calculate real activity data
    const storageKeys = [
        'soCompletedLectures', 
        'soCompletedQuizzes', 
        'soCompletedSections', 
        ...
        try {
            ...
            for (const id in data) {
                if (typeof data[id] === 'number') {
                    allCompleted.push(data[id]);
                }
            }
        } catch (e) {}
    });

    // 2. Prepare 14 days buckets
    const today = new Date();
    today.setHours(23, 59, 59, 999);
    
    // Arrays to hold count per day. Index 6 is today, 0 is 6 days ago.
    const currentWeekData = [0, 0, 0, 0, 0, 0, 0];
    const lastWeekData = [0, 0, 0, 0, 0, 0, 0];
    
    const ONE_DAY = 24 * 60 * 60 * 1000;

    allCompleted.forEach(timestamp => {
        const diffTime = today.getTime() - timestamp;
        const diffDays = Math.floor(diffTime / ONE_DAY);
        
        if (diffDays >= 0 && diffDays <= 6) {
            // Current week (0 is today -> index 6, 6 is 6 days ago -> index 0)
            currentWeekData[6 - diffDays]++;
        } else if (diffDays >= 7 && diffDays <= 13) {
            // Last week
            lastWeekData[13 - diffDays]++;
        }
    });

    // 3. Generate dynamic labels ending with today
    ...
    if (canvasContext) {
        ...
        gradient.addColorStop(0, 'rgba(14, 165, 233, 0.4)'); // brand-accent
        gradient.addColorStop(1, 'rgba(14, 165, 233, 0.0)');
    }

    // Find max value for Y-axis scaling
    const maxVal = Math.max(...currentWeekData, ...lastWeekData, 5);
            ...
            datasets: [
                {
                    ...
                    borderColor: '#0EA5E9', // brand-accent
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    titleColor: isDark ? 'rgba(255,255,255,0.9)' : 'rgba(15, 23, 42, 0.9)',
