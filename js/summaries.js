...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Force Emerald Theme Colors for this page
    document.documentElement.style.setProperty('--subject-accent', '#10b981');
    document.documentElement.style.setProperty('--subject-glow', '#6ee7b7');

    // Setup Hero
    document.getElementById('chap-subj-title').textContent = subject.title;

    // The background is now purely handled in summaries.html with the app-bg-container class.
    ...
    const chapters = (typeof getSubjectSectionData === 'function')
        ? getSubjectSectionData(subject, 'summaries')
        : ((subject.content && subject.content.summaries && subject.content.summaries.length > 0) ? subject.content.summaries : defaultChapters);
    ...
    const html = chapters.map((ch, chIndex) => {
            ...
            <div class="lecture-item ${isPdf ? 'is-pdf' : 'is-video'} ${isDone ? 'done' : ''}" ${clickHandler}>
                <div class="lec-info">
                    ...
                    <div class="lec-text-col">
                        ...
                        ` : `
                        <a href="player.html?id=${lec.id}&subjectId=${subjectId}&type=${lec.type}&url=${encodeURIComponent(lec.url)}&title=${encodeURIComponent(lec.title)}&sec=summaries${nextParams}" class="lec-open-btn" onclick="event.stopPropagation()">
                            Open
        ...
        <div class="chap-card group relative" onclick="toggleChapter(this)">
            <span class="chap-num font-heading font-black italic absolute -top-4 right-2 text-emerald-100 opacity-40 group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-500 pointer-events-none select-none text-[5rem] leading-none" style="text-shadow: 0 10px 30px rgba(16,185,129,0.1);">${String(ch.num).padStart(2, '0')}</span>
            ...
            <div class="chap-main-content relative z-10">
                <div class="chap-header flex items-start justify-between">
                    <div class="chap-header-left flex flex-col gap-1.5">
                        <span class="chap-ch-label text-[0.65rem] font-black uppercase tracking-[0.2em] text-emerald-500 bg-emerald-50/50 w-fit px-2.5 py-1 rounded-md border border-emerald-100/50">Part ${ch.num}</span>
                        <h2 class="chap-title-text font-heading text-[1.75rem] font-bold text-slate-800 leading-[1.15] tracking-tight group-hover:text-emerald-700 transition-colors">${ch.title}</h2>
                    </div>
                    <div class="chap-toggle-icon w-10 h-10 rounded-full bg-white/60 border border-white/80 shadow-[0_4px_10px_rgba(0,0,0,0.03)] flex items-center justify-center text-emerald-500 group-hover:bg-emerald-500 group-hover:text-white group-hover:shadow-[0_8px_20px_rgba(16,185,129,0.3)] transition-all duration-300 transform group-hover:scale-105">
                ...
                <div class="chap-meta">
                    <div class="chap-meta-item shadow-sm border border-emerald-50/50">
